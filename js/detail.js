        ...
        if (data) {
            ...
            if (detailContainer) {
                if (data.image) {
                    ...
                    // 為了讓大圖在詳情頁呈現最佳美感，使用 cover 填滿整個背景
            ...
            // 5.5 完整色譜
            const cardFullPalette = document.getElementById('card-full-palette');
            if (cardFullPalette) {
                const fullPaletteImg = document.getElementById('full-palette-img');
                if (data.image && fullPaletteImg) {
                    fullPaletteImg.src = data.image;
                    fullPaletteImg.style.display = 'block';
                } else {
                    cardFullPalette.style.display = 'none';
                }
            }
            ...
            if (elementContainer) {
                ...
                for (let i = 1; i <= 5; i++) {
                    ...
                    // 圖解區：原始元素 → 視覺圖騰
                    const hasText = eName || eDesc;
                    section.innerHTML = `
                        <div class="transform-graphic" style="display:flex; align-items:center; justify-content:center; gap:12px; ${hasText ? 'margin-bottom:12px;' : 'margin-bottom:0;'}">
                            <div style="display:flex; flex-direction:column; align-items:center; gap:6px;">
                                ${eOrigin
                                    ? `<img src="${eOrigin}" alt="原始元素" style="width:120px; height:80px; object-fit:contain; border-radius:4px;">`
                            ...
                            <div style="display:flex; flex-direction:column; align-items:center; gap:6px;">
                                ${eMotif
                                    ? `<img src="${eMotif}" alt="視覺圖騰" style="width:120px; height:80px; object-fit:contain; border-radius:4px;">`
                                    : `<div style="width:120px; height:80px; background:rgba(105,187,205,0.15); border-radius:4px;"></div>`}
                                <span class="body-12-18" style="opacity:0.55;">視覺圖騰</span>
                            </div>
                        </div>
                        ${eName ? `<h3 class="section-title" style="margin-bottom:8px;">${eName}</h3>` : ''}
                        ${eDesc ? `<p class="body-12-18 photo-caption" style="white-space:pre-wrap; margin-bottom:0;">${eDesc}</p>` : ''}
                    `;
